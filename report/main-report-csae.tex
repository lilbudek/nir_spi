...
void detectbuttons() {
  switch (button) {
	...
	Для ввода команды была написала функция append\_cmd (листинг 2.3). Подпрограмма  только принимает. На вход поступает, вводимый пользователем набор бит, а далее записывается в команду. Ход работы поясняется на рисунке 2.3. 
	\begin{code}
...
myOLED.update();
	...
	Полный код программы приведён в приложении. После успешной компиляции среда разработки вывело сообщение о том, что программа использует 6892 байт (21\%) памяти устройства. Всего доступно 32256 байт. %TODO
	\newpage
...
%TODO нужно хорошо описать процесс работы пользователя с устройством
	Процесс работы пользователя с устройством можно представить следующей структурной схемой (рис.4.1). 
	\begin{figure}[H]
	\begin{center}
	\includegraphics[scale=1]{struct} %TODO всё равно не нравится надо подумать как перерисовать лучше
	\end{center}	
